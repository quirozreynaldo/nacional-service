...
public class ProveedorMedicoServicio {
    ...
    private String descriptionFormat(ProveedorMedico proveedorMedico) {
        ...
        if(proveedorMedico.getUniqueId()!=null && proveedorMedico.getUniqueId().trim().length() > 0){
            replacements.put("idT_value",proveedorMedico.getUniqueId());
        }else {
    ...
    private String getWebLink() {
        return "https://es.research.net/r/XJ3NVP2?irub=[irub_value]&t=[t_value]&iciu=[iciu_value]&ser=[ser_value]&fcon=[fcon_value]&nsol=[nsol_value]&e=[e_value]&ieje=[ieje_value]&nase=[nase_value]&fden=[fden_value]&dia=[dia_value]&int=[int_value]&pro=[pro_value]&iprv=[iprv_value]&pol=[pol_value]&idT=[idT_value]";
    }
