    ...
    private void sendJira(List<VagonetaSegura> vagonetaSeguraList) {
        for (VagonetaSegura vagonetaSegura : vagonetaSeguraList) {
            try {
                ...
                manageLog.recorJiralog(Utils.createJiraLog(issue.getIssueId(),issue.getIssueKey(),issue.getRequestTypeId(),issue.getServiceDeskId(),Constant.CONFIG_VAGONETA_SEGURA,fileName));
