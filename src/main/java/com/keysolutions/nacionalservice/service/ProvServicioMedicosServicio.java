...
public class ProvServicioMedicosServicio {
    ...
    private void sendJira(List<ProvServicioMedico> provServicioMedicoList) {
        for (ProvServicioMedico provServicioMedico : provServicioMedicoList) {
            try {
                ...
                manageLog.recorJiralog(Utils.createJiraLog(issue.getIssueId(),issue.getIssueKey(),issue.getRequestTypeId(),issue.getServiceDeskId(),Constant.CONFIG_PROV_SERV_MEDICO,fileName));
    ...
    private String getWebLink() {
        return "https://es.research.net/r/DKYDG9J?irub=[irub_value]&iciu=[iciu_value]&ser=[ser_value]&fcon=[fcon_value]&nsol=[nsol_value]&t=[t_value]&e=[e_value]&ieje=[ieje_value]&nase=[nase_value]&fden=[fden_value]&dia=[dia_value]&int=[int_value]&pro=[pro_value]&iprv=[iprv_value]&pol=[pol_value]&idT=[idT_value]";
    }
