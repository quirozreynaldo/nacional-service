    ...
    private void sendJira(List<CallCenterNspf> callCenterNspfList) {
        for (CallCenterNspf callCenterNspf : callCenterNspfList) {
            try {
                ...
                manageLog.recorJiralog(Utils.createJiraLog(issue.getIssueId(),issue.getIssueKey(),issue.getRequestTypeId(),issue.getServiceDeskId(),Constant.CONFIG_CALLCENTER_NSPF,fileName));
